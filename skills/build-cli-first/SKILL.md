...
### 4. Offer to build CLI feature first (if available)
If the project has a CLI or logic layer that can be built before the UI, offer to implement that first for faster feedback loops.
- If the UI is designed to gather user inputs, offer the CLI option to read a CSV or JSON file to simulate user input.

### 5. Offer a feature branch
...
### 10. Blockers during implementation
